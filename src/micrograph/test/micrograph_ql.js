var MicrographQuery = require("./../src/micrograph_query").MicrographQuery;
...
exports.bgp1 = function(test) {
    ...
    var pattern = MicrographQuery.prototype._parseQuery({name: "john"});
    var parsedQuery = aqt.parseQueryString("SELECT ?id0 ?id0p ?id0o { ?id0 <name> \"john\" ; ?id0p ?id0o . }");
    test.ok(pattern.varsMap['id0'] === 'id0');
    console.log(sys.inspect(pattern.query,true,20));
    console.log(sys.inspect(parsedQuery,true,20));
...
exports.bgp2 = function(test) {
    MicrographQL.counter = 0;
    MicrographQL.counter = 0;
    var pattern = MicrographQuery.prototype._parseQuery({name: "john", friend: {name: "mary"}});
