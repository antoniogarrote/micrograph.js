...
var TabulatorN3Parser = function() {
	...
	var lit = {'token':'literal', 'value': str};

	if (this.datatype){
	    lit['type'] = this.datatype.value;
	...
	*/
	...
	try {
		...
		var p = N3Parser.N3Parser(kb, kb, null, graph, null, null, "", null);
		p.loadBuf(str);
		return;
            }
	} catch(e) {
...
/*
[{"subject":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/1992_European_Super_Cup"},"predicate":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/property/stadium"},"object":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/Barcelona"},"graph":"http://dbpedia.org/data/Barcelona.n3"},{"subject":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/1999_Supercopa_de_Espa%C3%B1a"},"predicate":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/property/stadium"},"object":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/Barcelona"},"graph":"http://dbpedia.org/data/Barcelona.n3"},{"subject":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/2006_Supercopa_de_Espa%C3%B1a"},"predicate":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/property/stadium"},"object":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/Barcelona"},"graph":"http://dbpedia.org/data/Barcelona.n3"},{"subject":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/2010%E2%80%9311_EHF_Champions_League"},"predicate":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/property/stadium"},"object":{"token":"uri","prefix":null,"suffix":null,"value":"http://dbpedia.org/resource/Barcelona"},"graph":"http://dbpedia.org/data/Barcelona.n3"}]
*/
...
var compactPredicate = function(predicate) {
...
var toNodes = function(triples,options) {
    var acum = {};
    var toReturn = [];
    var blankMap = {};
    var currNs = {};
    ...
    if(options['ns']) {
	for(var p in options['ns'])
	    currNs[options['ns'][p]] =  p;
	for(p in defaultNs) 
	    currNs[defaultNs[p]] =  p;
    }

    var triple,subject, predicate, object, node, nextId;
	...
	if(predicate === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type')
	    predicate = '$type';
	if(options['compactProperties'] === true){
	    predicate = compactPredicate(predicate);
	} else {
	    for(var prefix in currNs) {
		if(predicate.indexOf(prefix) === 0) {
		    predicate = predicate.replace(prefix, currNs[prefix]+":");
		    break;
		}
	    }
	}
	...
	if(subject.blank != null) {
	    nextId = "blank"+mg.blank;
	    mg.blank++;
	    subject = nextId;
	    blankMap[mg['blank']] = nextId;
	} else {
	...
	if(object.token === "uri") {
	    ...
	    if(object.type && object.type.indexOf("int") != -1 ||
	       object.type && object.type.indexOf("float") != -1 ||
	       object.type && object.type.indexOf("decimal") != -1) {
		object = parseFloat(object.value);
	    } else if(object.type && object.type.indexOf("bool") != -1) {
		object = (object.value === "true");
	    } else if(object.lang == null || object.lang === language) {
		object = object.value
	    } else if(options["lang"]) {
		object = null;
	    }
	} else if(object.blank != null ) {
	    if(blankMap[object['blank']] != null)
		object = {'$id': blankMap[object['blank']]};
	    else {
		nextId = "blank"+mg.blank;
		mg.blank++;
		object = {'$id': nextId};
	...
	if(object != null) {
	    ...
	    } else if(node[predicate].constructor === Array) {
		node[predicate].push(object);
	    } else {
		node[predicate] = [node[predicate], object];
	    }
	}
    }
...
var parse = function(data, graph, opts) {
    ...
    var quads = g.toQuads();
    return toNodes(quads, opts);
};

mg['n3'] = function(baseUri, data, options) {
    return parse(data,baseUri, options);
};